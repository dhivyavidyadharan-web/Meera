...
function formatScoresHtml(ranked) {
  const lines = ranked.map(
    (k, i) =>
      `${i + 1}. <b>${escapeHtml(k.keyword)}</b>: ${k.total}  (news ${k.news}, ${k.articleCount} articles | voice fit ${k.fit})`
  );
  return [
    "<b>KEYWORD SCORES</b>",
    ...
    ...lines,
    "",
    `Writing around: <b>${escapeHtml(ranked[0].keyword)}</b>`,
  ].join("\n");
}
