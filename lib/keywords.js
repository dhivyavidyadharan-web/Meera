  ...
  properties: {
    keywords: {
      ...
      items: {
        ...
        required: ["keyword", "voice_fit", "fit_reason"],
      },
    },
  },
  required: ["keywords"],
...
async function extractKeywords(turns) {
...
async function scoreKeywords(turns) {
  const extracted = await extractKeywords(turns);
...
function formatScores(ranked) {
  const lines = ranked.map(
    (k, i) =>
      `${i + 1}. ${k.keyword}: ${k.total}  (news ${k.news}, ${k.articleCount} articles | voice fit ${k.fit})`
  );
  return [
    "Keyword scores",
    "50% Google News volume + recency (7 days), 50% fit with your voice",
    "",
    ...lines,
    "",
    `Writing around: ${ranked[0].keyword}`,
  ].join("\n");
}

module.exports = { scoreKeywords, formatScores };
