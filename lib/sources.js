...
async function factCheck(post, headlines, deadline) {
...
function formatRelatedHtml(articles, { anyCited }) {
  const heading = anyCited
    ? "<b>MORE NEWS ON THIS TOPIC</b> <i>(real Google News results, not cited in the post)</i>"
    : "<b>NEWS FOUND FOR THIS TOPIC</b> <i>(real Google News results - the post does not cite them)</i>";
  return [
    heading,
    ...articles.map((a, i) => {
      const publisher = a.sourceUrl ? link(a.sourceUrl, hostOf(a.sourceUrl) || a.source) : escapeHtml(a.source);
...
const NO_NEWS_HTML =
  "<b>SOURCES</b>\n<i>No Google News articles from the last 7 days matched these keywords, so this post cites no news sources.</i>";
