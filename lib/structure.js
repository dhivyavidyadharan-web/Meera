...
function findProblems(text, { minSources = 0, citedSources = 0 } = {}) {
  ...
  if (total > MAX_WORDS) {
    problems.push(
      `The post is ${total} words; the limit is ${MAX_WORDS}. Cut at least ${total - 330} words: drop background and repeated points, keep exactly 2 factors, and remove the Skinstinct line if needed.`
    );
  }
...
const postWordCount = (text) => words(splitNote(text).body);

module.exports = { findProblems, tidy, sentences, postWordCount, MAX_WORDS };
