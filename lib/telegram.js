...
// HTML messages must not be chunked by raw length: link URLs inflate it, and a
// split can cut a tag in half. Callers check visibleLength() instead.
async function sendMessage(chatId, text, { noChunk = false, ...options } = {}) {
  const chunks = noChunk ? [text] : chunkMessage(text);
...
async function sendHtml(chatId, html, options = {}) {
  try {
    await sendMessage(chatId, html, {
      ...
      noChunk: visibleLength(html) <= TELEGRAM_MESSAGE_LIMIT,
      ...options,
    });
  } catch (err) {
