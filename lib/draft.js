...
// Skip the final fact-check if we're this far into Vercel's 60s limit.
const FINAL_CHECK_DEADLINE_MS = 38000;
...
async function audit(post, citations, headlines, { runFactCheck = true } = {}) {
  ...
  if (runFactCheck) {
    try {
      factIssues = (await factCheck(post, headlines)).map(describeIssue);
      factCheckRan = true;
    } catch (err) {
...
async function writePost({ system, turns, note, topic, headlines, startedAt }) {
  ...
  const first = await generate({ system, turns: draftTurns, jsonSchema: DRAFT_SCHEMA });
  ...
  const timeLeft = Date.now() - startedAt < FINAL_CHECK_DEADLINE_MS;
  const finalAudit = await audit(post, citations, headlines, { runFactCheck: timeLeft });
  const needsChecking = [...finalAudit.citationProblems, ...finalAudit.factIssues];
  if (!finalAudit.factCheckRan) {
    needsChecking.push("The final fact-check did not run (time limit or error). Cross-check each claim against the sources before posting.");
  }
