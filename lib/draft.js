...
async function writePost({ system, turns, note, topic, headlines, startedAt }) {
  ...
  if (toFix.length === 0 || left() < NEED_FOR_REVISION_MS) {
    const needsChecking = [...firstAudit.citationProblems, ...firstAudit.factIssues, ...lengthWarning(post)];
  ...
  const needsChecking = [...finalAudit.citationProblems, ...finalAudit.factIssues, ...lengthWarning(post)];
  if (!finalAudit.factCheckRan) needsChecking.push(SKIPPED_CHECK);

  return { post, sources: finalAudit.valid, needsChecking };
}

function lengthWarning(post) {
  const n = postWordCount(post);
  return n > MAX_WORDS ? [`The post is ${n} words, over the ${MAX_WORDS}-word limit. Trim ${n - MAX_WORDS}+ words before posting.`] : [];
}
