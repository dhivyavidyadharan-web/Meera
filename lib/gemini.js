...
async function generate({ system, turns, temperature = 0.8, jsonSchema, deadline, fast = false, thinkingBudget }) {
  ...
  const buildBody = () =>
    JSON.stringify({
      ...
      generationConfig: {
        ...
        ...(thinkingBudget != null && !thinkingUnsupported.has(`${model}:${thinkingBudget}`)
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    // Fall back instead of failing. Gemini rejects unsupported settings with a
    // generic 400 "invalid argument", so on any 400 first drop the thinking
    // budget, then leave the fast model; each fallback is remembered.
    const thinkingKey = `${model}:${thinkingBudget}`;
    const sentThinking = thinkingBudget != null && !thinkingUnsupported.has(thinkingKey);
    if (res.status === 400 && sentThinking) {
      console.error(`Gemini 400 on ${model} with thinkingBudget=${thinkingBudget}; retrying without it`);
      thinkingUnsupported.add(thinkingKey);
      attempt--;
      continue;
    }
    if (model === FAST_MODEL && [400, 403, 404].includes(res.status)) {
      console.error(`Fast model ${FAST_MODEL} failed (${res.status}); using ${MAIN_MODEL}`);
